#include "controllers/admin/AdminBookController.h"
...
void AdminBookController::handleUpdateBook(const httplib::Request &req, httplib::Response &res) const {
    ...
    auto getStringOrOld = [&](const nlohmann::json &reqData, const string &oldValue) {
...
void AdminBookController::handleDeleteBook(const httplib::Request &req, httplib::Response &res) const {
    ...
    if (inventoryService->deleteBook(bookId) && inventoryService->deleteAllBookCopy(bookId)) {
        const nlohmann::json responseData = {
            {"success", true},
            {"message", "图书删除成功"}
        };
...
void AdminBookController::handleGetAllBookCopies(const httplib::Request &req, httplib::Response &res) const {
    ...
    const vector<BookCopy> copies = inventoryService->getBookCopies(bookId);
...
void AdminBookController::handleManualBorrowBookCopy(const httplib::Request &req, httplib::Response &res) const {
    Logger::getInstance().logAccess("POST /api/admin/borrow/manual 人工借书");

    nlohmann::json requestData = HttpUtils::parseRequestBody(req);
    string userId = requestData["userId"];
    string copyId = requestData["borrowCopyId"];
    ...
    if (!inventoryService->isCopyBookIdExist(copyId)) {
        res = HttpUtils::createErrorResponse("图书副本不存在", 404);
        return;
    }
