...
string PasswordUtils::hashPassword(const string &password, const string &salt) {
    // 将密码和盐值组合
    const string combined = password + salt;
...
string PasswordUtils::encryptPassword(const string &password) {
    ...
    // 计算哈希值
    const string hash = hashPassword(password, salt);
...
bool PasswordUtils::verifyPassword(const string &password, const string &encryptedPassword) {
    // 分割盐值和哈希值
    const size_t colonPos = encryptedPassword.find(':');
    if (colonPos == string::npos) {
        // 格式错误
        return false;
    }

    const string salt = encryptedPassword.substr(0, colonPos);
    const string storedHash = encryptedPassword.substr(colonPos + 1);

    // 使用相同的盐值计算输入密码的哈希值
    const string calculatedHash = hashPassword(password, salt);
