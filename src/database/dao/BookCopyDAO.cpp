...
//删除图书副本
bool BookCopyDAO::deleteBookCopy(const std::string &copyId) const {
    const std::string sql = "DELETE FROM book_copy WHERE copy_id = ?;";
...
//删除指定图书的所有图书副本
bool BookCopyDAO::deleteAllBookCopy(const std::string &bookId) const {
    const std::string sql = "DELETE FROM book_copy WHERE book_id = ?;";
...
//更新图书副本状态
bool BookCopyDAO::updateBookCopyStatus(const std::string &copyId, const std::string &status) const {
    const std::string sql = "UPDATE book_copy SET status = ? WHERE copy_id = ?;";
...
int BookCopyDAO::getCopyCountByBookId(const std::string &bookId) const {
    const std::string sql = "SELECT COUNT(*) FROM book_copy WHERE book_id = ?;";
    sqlite3_stmt *stmt = nullptr;
    if (!bookCopyDatabase->prepare(sql, &stmt)) {
...
int BookCopyDAO::getAvailableCopyCount(const std::string &bookId) const {
    const std::string sql = "SELECT COUNT(*) FROM book_copy WHERE book_id = ? AND status = 'available';";
    sqlite3_stmt *stmt = nullptr;
    if (!bookCopyDatabase->prepare(sql, &stmt)) {
...
//根据图书id获取所有副本
std::vector<BookCopy> BookCopyDAO::getCopiesByBookId(const std::string &bookId) const {
...
//根据图书id获取所有可借阅的副本
std::vector<BookCopy> BookCopyDAO::getAvailableCopies(const std::string &bookId) const {
    std::vector<BookCopy> copies;
    const std::string sql =
            "SELECT copy_id, book_id, status "
            "FROM book_copy "
...
//根据副本id判断图书是否已借出
bool BookCopyDAO::isCopyBorrowed(const std::string &copyId) const {
    const std::string sql = "SELECT COUNT(*) FROM book_copy WHERE copy_id = ? AND status = 'borrowed';";
    sqlite3_stmt *stmt = nullptr;
